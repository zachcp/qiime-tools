...
def parallel_split_library(fastq, qualfile, outfile, mappingfile, barcodetype,qual_cutoff, logfile,
    ...
    # two-line files.
    print("Splitting the Fasta File, {}".format(fastq))
    if isgzip(fastq):
        call("zcat {} | seqtk seq -l0 | split -l {} - {}".format(fastq, splitsize ,"forward_"),shell=True)
    else:
        call("cat {}  | seqtk seq -l0 | split -l {} - {}".format(fastq, splitsize ,"forward_"),shell=True)

    print("Splitting the Qual  File, {}".format(qualfile))
    if isgzip(qualfile):
        call("zcat {} | seqtk seq -l0 | split -l {} - {}".format(qualfile, splitsize,"barcode_"),shell=True)
    else:
        call("cat {}  | seqtk seq -l0 | split -l {} - {}".format(qualfile, splitsize,"barcode_"),shell=True)
    ...
    handlerfunc = partial(process_split_files, splitlibrarycommand=splitlibrarycommand,
                          ...
                          splitsize=splitsize, qualwindow=qualwindow,
