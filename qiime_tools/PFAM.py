...
def getUniprotIDsfromPFAM(PFAMID, keepfiles=True):
    ...
    The PFAM ID guides the followign set of lookups:

        1. PFAMID -> PFAM Alignment Download (Stockholm Format)
        2. Parse PFAM file -> Uniprot ID Start/Stop for each member in the PFAM alignment
        3. UniprotID -> European Nucleic Acid ID
        4. ENA ID -> Nucelic Acid Fasta file.
        5. Nucleic Acid Fasta + Uniprot Start/stop -> Nucleic Acid Refence locations

    :param keepfiles: if True, will keep the intermediated files generated by this script.
    """
    ...
    try:
        ...
        raise ValueError("Could not Download the PFAM File")
